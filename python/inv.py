...
def inv_func(comp_va):
    #カレントディレクトリ
    directory = os.listdir('/etc/ansible/python/')
    #比較対象となる過去の抽出データ
    # zabbix.txtがカレントディレクトリにあるか確認ある場合は、下記の処理
    if 'zabbix.txt' in directory:
        hikakudata = ''
        f = open('/etc/ansible/python/zabbix.txt', 'r')  # zabbix.txtというファイル名でカレントディレクトリに保存
        for i in f:
        ...
        if hikakudata != str_comp:
            #現在の監視対象データをファイル保存する
            f = open('/etc/ansible/python/zabbix.txt', 'w')  # zabbix.txtというファイル名で新たにカレントディレクトリに保存
            f.write(str(comp_va))
            f.close()
            #現在の監視対象データをforする
            for ip in comp_va:
            ...
            f = open('/etc/ansible/csr1000v-inv', 'w')  # invというファイル名でカレントディレクトリに保存
            f.write(output)
            f.close()
            #slackAPI用URL
